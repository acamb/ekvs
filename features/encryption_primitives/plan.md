...
5. **Implement `DeriveKey` — RSA branch**
   Handle `*rsa.PrivateKey`: concatenate `key.Primes[0].Bytes()` and `key.Primes[1].Bytes()` (P ∥ Q)
   as the IKM, pass through the same HKDF call.
...
8. **Write unit tests (table-driven) in `crypto_test.go`**
   Cover:
...
9. **Run `go mod tidy` and validate**
   ...
   then `make test`; confirm `ok  ekvs/internal/crypto`.
