...
- [ ] `internal/storage/errors.go` exports exactly `ErrProjectNotFound`, `ErrProjectAlreadyExists`, `ErrKeyNotFound`, `ErrInvalidName`.
...
| # | Input | Expected |
|---|---|---|
| LP-1 | User with no projects | Empty slice, no error |
...
| C-3 | Concurrent reads while one goroutine writes to the **same** project | Reads return consistent data; no races |
